...
| 项目                  | <div style="width: 125px;">状态</div> | 说明                                                                                                                |
|---------------------|---------------------------------------|---------------------------------------------------------------------------------------------------------------------|
| IPv4 / IPv6         | ✅ 已支持                             | 启动时自动检测 IPv6，可用则启用，不可用则自动降级到 IPv4。可修改merlin-box.sh中的MB_ENABLE_IPV6变量值直接禁用IPV6。 |
| IPv4 / IPv6 分流      | ✅ 已支持                             | ipset + iptables/ip6tables                                                                                          |
| LAN 侧 QUIC（UDP 443） | ✅ 已处理                             | 默认拦截（DROP UDP 443），避免 UDP 直连泄露。可修改merlin-box.sh中的MB_DISABLE_QUIC_FROM_LAN变量禁用拦截功能。      |
